...
exports.scrapeAndSaveCompanyData = async (req, res) => {
  try {
    ...
    if (existingCompany) {
      return res.status(200).json({ message: "Company data already exists for this URL", data: existingCompany });
    }
    const newCompany = new Company(companyData);
    await newCompany.save();
    return res.status(200).json({ message: "Company data saved successfully", data: newCompany });
  } catch (err) {
    res.status(500).json({ message: "Error scraping or saving data", error: err.message });
  }
};
...
exports.deleteMultipleCompanies = async (req, res) => {
  try {
    ...
    if (!ids || !Array.isArray(ids) || ids.length === 0) {
      return res.status(400).json({ message: "Invalid request. Please provide an array of company IDs.", });
    }
    const result = await Company.deleteMany({ _id: { $in: ids } });
    if (result.deletedCount == 0) {
      return res.status(404).json({ message: "No companies found for the provided IDs." });
    }
    return res.status(200).json({ message: "Companies deleted successfully", deletedCount: result.deletedCount });
  } catch (err) {
